...
class PolylangTranslateExistingMedia
{
    ...
    public function __construct()
    {
        ...
        if (isset($polylang)) {
            ...
            add_action('admin_init', [&$this, 'translate_existing_media']);
    ...
    function translate_existing_media()
    {
        ...
        $this->custom_fields = apply_filters('polylang-translate-existing-media-custom-fields-with-image-id', []);

        // search for custom fields with regex
        $this->custom_fields_regex = apply_filters('polylang-translate-existing-media-custom-fields-regex-with-image-id', '');
        ...
        if (! PLL()->model->options['media_support']) {
            ...
            $paged = ($_GET["polylang-translate-existing-media-library-p"]) ? $_GET["polylang-translate-existing-media-library-p"] :
                1; // Paged because running too many posts at a time causes memory overflow
            ...
            if (! empty($next_page)) {
                ...
                $this->admin_notice .= '<a class="button" style="margin:-.15rem 0 .25rem .25rem" href="' . $full_url . '">Continue media library translation (step ' . $next_page
                                       . ' / ' . $max_num_pages . ')</a>';
            } else {
                ...
                $this->admin_notice .= '<br /><br />Done! Media library is translated. Next, replace all existing images from content. <a class="button" style="margin:-.15rem 0 .25rem 0" href="'
                                       . $replace_full_url . '">2. Translate existing images in content</a>';
            }
            ...
            $skip_post_types = apply_filters('polylang-translate-existing-media-skip-post-types',
                ['attachment', 'revision', 'acf-field', 'acf-field-group', 'nav_menu_item', 'polylang_mo']);
            ...
            $paged = ($_GET["polylang-translate-existing-media-p"]) ? $_GET["polylang-translate-existing-media-p"] :
                1; // Paged because running too many posts at a time causes memory overflow
            ...
            while ($query->have_posts()) : $query->the_post();
                ...
                $all_custom_fields = $this->custom_fields;

                if (! empty($this->custom_fields_regex)) {
                    $post_metas        = get_post_meta($query->post->ID);
                    $matches           = preg_grep($this->custom_fields_regex, array_keys($post_metas));
                    $all_custom_fields = array_merge($all_custom_fields, $matches);
                }
            ...
            if (! empty($next_page)) {
                ...
                $this->admin_notice .= '<a class="button" style="margin:-.15rem 0 .25rem .25rem" href="' . $full_url . '">Continue replacing existing images in content (step '
                                       . $next_page . ' / ' . $max_num_pages . ')</a>';
            } else {
            ...
            if (! empty($this->custom_fields)) {
                $this->admin_notice .= '<br /><br />You have added following custom fields that have image IDs: <span style="display:block;background: #f1f1f1;padding:.5rem;margin-bottom:1rem;">'
                                       . esc_attr(implode(', ', $this->custom_fields)) . '</span>';
            }

            if (! empty($this->custom_fields_regex)) {
                $this->admin_notice .= '<br /><br />You have added following regex for custom fields that have image IDs: <span style="display:block;background: #f1f1f1;padding:.5rem;margin-bottom:1rem;">'
                                       . esc_attr($this->custom_fields_regex) . '</span>';
            }

            $this->admin_notice .= '<a class="button" style="margin:-.15rem .5rem .25rem 0" href="' . $translate_full_url
                                   . '">1. Translate the whole media library</a><a class="button" style="margin:-.15rem 0 .25rem 0" href="' . $replace_full_url
                                   . '">2. Translate existing images in content</a>';
    ...
    function replace_content_img($content, $post, $new_lang_slug)
    {
        ...
        for ($i = 0; $i < count($img_array[0]); $i++) {
            $img_and_meta[$i] = ['tag' => $img_array[0][$i]];
        }
    ...
    function replace_content_caption($content, $post, $new_lang_slug)
    {
        ...
        for ($i = 0; $i < count($caption_array[0]); $i++) {
            $caption_and_meta[$i] = ['shortcode' => $caption_array[0][$i]];
        }
        ...
        foreach ($caption_and_meta as $i => $arr) {
            ...
            if (! empty($caption_and_meta[$i]['txt'])) {
                ...
                if (! empty($caption_and_meta[$i]['new_txt'])) {
                    ...
                    $caption_and_meta[$i]['new_shortcode'] = preg_replace('/ \/>(.*?)\[\/caption\]/i', '/>' . $caption_and_meta[$i]['new_txt'] . '[/caption]',
                        $caption_and_meta[$i]['new_shortcode']);
                }
    ...
    function replace_content_gallery($content, $post, $new_lang_slug)
    {
        ...
        for ($i = 0; $i < count($gallery_array[0]); $i++) {
            $gallery_and_meta[$i] = ['shortcode' => $gallery_array[0][$i]];
        }
